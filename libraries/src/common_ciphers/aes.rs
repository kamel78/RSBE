...
pub struct AES128 {
    round_keys: [[u8; 16]; 11], // 11 round keys (original + 10 rounds)
}
    ...
    fn key_expansion(&mut self, key: u128) {        
        ...
        for round in 1..11 {
            ...
            for i in 1..4 {
                ...
                let prev_base = (i - 1) * 4;
                for j in 0..4 {
                    new_key[base + j] = new_key[prev_base + j] ^ prev_key[base + j];
                }
            }
...
impl CipherInterface for AES128{

    type Cipher = AES128;

    fn name(&self) -> &'static str {AES128::NAME}
